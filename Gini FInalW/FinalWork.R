...
data_arab = data[data$national == 2, ]  # Create data frame with dummies for both groups
data_jewish = data[data$national == 1, ]
cons_arab = lm(c3 ~ incmonet + hhprsns + married + divorce + separated, data_arab)  # Those 2 models are the unrestricted models 
cons_jewish = lm(c3 ~ incmonet + hhprsns + married + divorce + separated, data_jewish)
####


plot(c3~incmonet,data,xlim=c(0, 60000) ,pch=6, cex=c(0.5,0),col="dodgerblue")
abline
plot(inc~decile ,data,col="tomato", pch=6, cex=c(0.8,0))

plot(main="School Years~Log of Wage per Hour" , xlab="School Years", ylab="Log of Wage per Hour",
     col="tomato", xlim=c(0, 25), ylim=c(0, 7), pch=6, cex=c(0.9,0))
